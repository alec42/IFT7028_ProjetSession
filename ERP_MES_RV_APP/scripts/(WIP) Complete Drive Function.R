...
Commande <- read_sheet(link_gs_erp, sheet = customerOrdersSheetName)
Inventaire <- read_sheet(link_gs_erp, sheet = InventorySheetName)
CommandesFournisseurs <- read_sheet(link_gs_erp, sheet = purchaseOrdersSheetName)
PanneauDetail <- read_sheet(link_gs_erp, sheet = panneauxSheetName)
Items <- read_sheet(link_gs_erp, sheet = itemsSheetName)
DisposUsine <- read_sheet(link_gs_erp, sheet = factoryHoursSheetName) %>% mutate(across(c(Monday, Tuesday, Wednesday, Thursday, Friday), ~format(., format="%H:%M:%S")))
DisposEmployers <- read_sheet(link_gs_erp, sheet = employeesDispoSheetName)
today <- Sys.Date()


# PiecesDetail
piecesDetail <- read_sheet(link_gs_erp, sheet = "PiecesDetail")

c(1, 2, 3, 4) + max(piecesDetail$PanneauID)
c(1, 2, 3, 4) + max(piecesDetail$PieceID)
